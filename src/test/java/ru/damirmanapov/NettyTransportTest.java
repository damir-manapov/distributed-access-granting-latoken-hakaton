...
import ru.damirmanapov.messages.Test2Message;
import ru.damirmanapov.messages.TestMessage;
import ru.damirmanapov.messages.WrapperMessage;
...
public class NettyTransportTest extends ConcurrentTestCase {
    ...
    @Test
    public void testSendingFewDifferentObject() throws TimeoutException {
        ...
        Serializer serializer = new Serializer(new UnpooledHeapAllocator());
        serializer.register(CatalystSerializable.class, 1);
        serializer.register(TestMessage.class, 2);
        serializer.register(Test2Message.class, 3);
        ThreadContext context = new SingleThreadContext("test-thread-%d", serializer);
        ...
        context.executor().execute(() -> {
            try {
                server.listen(new Address(new InetSocketAddress(InetAddress.getByName("localhost"), 4444)), connection -> {
                    connection.handler(TestMessage.class, testMessage -> {
                        threadAssertEquals("TestMessage", ((TestMessage) testMessage).getName());
                        System.out.println(testMessage);
                        return CompletableFuture.completedFuture("Hello world back!");
                    });

                    connection.handler(Test2Message.class, testMessage -> {
                        threadAssertEquals("Test2Message", ((Test2Message) testMessage).getName());
                        System.out.println(testMessage);
                        return CompletableFuture.completedFuture("Hello world back!");
                    });
                }
        ...
        context.executor().execute(() -> {
            try {
                ...
                Test2Message testMessage = new Test2Message("Test2Message");
